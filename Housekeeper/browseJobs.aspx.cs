    ...
    public partial class browseJobs : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            foreach (ListViewItem item in lvJobs.Items)
            {
                ...
                if (!IsDeadlineValid(jobId))
                {
                    btnApply.Text = "Deadline Passed";
                }               
            }
        ...
        protected void lnkApply_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                else if (!IsDeadlineValid(job_id))
                {
                    btn.Text = "Deadline Passed";
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "Pop", "viewMessageDeadlineModal();", true);
                }
                else
                {
                    SqlConnection con = new SqlConnection(_conString);
        ...
        private Boolean IsDeadlineValid(int jobID)
        {
            using (SqlConnection con = new SqlConnection(_conString))
            {
                ...
                {
                    cmd.CommandType = CommandType.Text;
                    cmd.CommandText = "SELECT Job_Deadline FROM tblJob WHERE Job_ID=@jobid";
                    cmd.Connection = con;
                    ...
                    using (SqlDataReader dr = cmd.ExecuteReader())
                    {
                        if (dr.Read())
                        {
                            ...
                            DateTime jobDeadline = dr.GetDateTime(dr.GetOrdinal("Job_Deadline"));

                            return jobDeadline >= DateTime.Now;
                        }
                        else
                        {
                            return false;
